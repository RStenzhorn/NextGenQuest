...
import static de.rjst.questsystem.config.CacheNames.MOVE;

@Service
public class MoveCacheImpl implements MoveCache {


    @Cacheable(value = MOVE, key = "#player.getUniqueId()")
    @Override
    ...
    @Override
    @CachePut(value = MOVE, key = "#player.getUniqueId()")
    public BigDecimal update(final Player player, final BigDecimal distance) {
        return distance;
    }

    @Override
    @CacheEvict(value = MOVE, key = "#player.getUniqueId()")
