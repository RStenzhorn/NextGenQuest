...
@Service("selectWeeklyQuestConfigSupplier")
public class SelectWeeklyQuestSelectionConfigSupplier implements Supplier<Optional<QuestSelectionConfigEntity>> {
    ...
    public Optional<QuestSelectionConfigEntity> get() {
        ...
        final Integer amount = propertySupplier.apply(NgqProperty.QUESTS_WEEKLY_AMOUNT, Integer.class);
        ...
        if (!ids.isEmpty()) {
            ...
            final TimePeriod period = new TimePeriodImpl(intervalType, TimeUtil.getStartDateWeek(fixedTime), TimeUtil.getEndDateWeek(fixedTime));
